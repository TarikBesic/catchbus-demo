...
function App() {
  ...
  useEffect(() => {
    findNearestStations()
  }, [stanice, firstLocation, secondLocation, nearestA, nearestB])
  ...
  useEffect(() => {
    if(nearestA && nearestB) {
      getBus()
      console.log('TRIGGERED', nearestA.name)
    }
  }, [nearestA, nearestB])
  ...
  return (
    <div className="app">
      <div className="map">
        <Map stanice = {stanice} busevi = {busevi} foundedBus = {foundedBus} firstLocation = {firstLocation} setFirstLocation = {setFirstLocation} secondLocation = {setSecondLocation} nearestA = {nearestA} nearestB = {nearestB} />
      </div>
    </div>
  );
}
