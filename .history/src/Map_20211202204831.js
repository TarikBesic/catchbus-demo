...
function Map({ ready, stanice, foundedBus, firstLocation, setFirstLocation, secondLocation, nearestA, nearestB }) {
    ...
    useEffect(() => { //when the bus gets founded requesting the route of the bus and walking path
        ...
        console.log(ready)
    }, [foundedBus, ready, firstLocation, secondLocation])

    const matchRoute = (e) => { //Bus route
        const url = "https://api.mapbox.com/matching/v5/mapbox/driving/" + e + "?geometries=geojson&overview=full&access_token=" + map_key;
        fetch(url).then(response => {
            return response.json();
        }).then(json => {
            console.log(json.matchings[0].geometry.coordinates)
            const coords = json.matchings[0].geometry.coordinates
            setApiCoords(coords)
        })
    }
    
    const matchWalking = (e) => { //User's walking path
        const url = "https://api.mapbox.com/matching/v5/mapbox/cycling/" + e + "?geometries=geojson&overview=full&access_token=" + map_key;
        fetch(url).then(response => {
            return response.json();
        }).then(json => {
            const coords = json.matchings[0].geometry.coordinates
